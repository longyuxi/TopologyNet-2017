...
def job(key):
    ...
    # {'error': 'False', 'finished': 'False', 'pearson': '0', 'rmse': '0', 'learning_rate': '0.01', 'seed': '3017369514048357217', 'prediction_results_file': '/usr/project/dlab/Users/jaden/tnet2017-new/svm/gbr_reruns/additional_results/gbr_reduce_0_predictions.npy', 'r2': '0', 'train_ratio': '0.9', 'scatter_plot_file': '/usr/project/dlab/Users/jaden/tnet2017-new/svm/gbr_reruns/additional_results/gbr_reduce_0_scatter_plot.html', 'num_predictors': '1', 'attempted': 'True'}
    ...
    num_predictors = int(d['num_predictors'])
    ...
    regr = GradientBoostingRegressor(n_estimators=num_predictors, learning_rate=learning_rate, random_state=seed, verbose=1)
    print('Starting fit')
    ...
    fig = px.scatter(x=y_test, y=y_hat, labels={'x': 'True binding affinity', 'y': 'Predicted binding affinity'}, title=f'True vs Predicted binding affinity\nR^2 = {r2_score(y_test, y_hat):.2f}, n = {len(y_test)}, RMSE: {np.sqrt(np.mean((y_test - y_hat)**2)):.2f}, Pearson: {np.corrcoef(y_test, y_hat)[0, 1]:.2f}, num_predictors: {num_predictors}, learning_rate: {learning_rate}')
