...
KEY_PREFIX = 'gbr_reduce_' # Prefix of every job, as appears in the Redis database
TRAIN_RATIO = 0.9
CLUSTER = 'CS' # or 'DCC'
...
if CLUSTER == 'CS':
    ...
    ROOT_DIR = '/usr/project/dlab/Users/jaden/tnet2017-new/svm/gbr_reruns'
...
def populate_db(rebuild_all_keys=False):
    ...
    # 1. Number of predictors (exponential from 1 to 100)
    num_predictors = [int(2**i) for i in range(7)]
    # 2. Learning rate
    learning_rates = [0.01, 0.03, 0.1, 0.3, 1, 3, 10]

    parameter_pairs = [(n, lr) for n in num_predictors for lr in learning_rates]

    keys = [KEY_PREFIX + str(i) for i in range(len(parameter_pairs))]
    ...
    for i in tqdm(range(len(keys))):
        ...
        DB.hset(k, mapping={
            ...
            'rmse': '0',
            'pearson': '0',
            'num_predictors': str(parameter_pairs[i][0]),
            'learning_rate': str(parameter_pairs[i][1]),
        })
