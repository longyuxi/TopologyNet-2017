...
def filter_out_nonexistent_entries(index, homology_base_folder = 'computed_homologies'):
    ...
    for idx in range(len(index)):
        ...
        if not pw_opposition.is_file():
            drop_row = True
            # print(f'no op for {item_base_folder}. File: {pw_opposition}')
...
class ProteinPersistenceHomologyDataset(Dataset):
    def __init__(self,
        ...
        print(f'Homology base folder: {homology_base_folder}')
        print(f'length of current index {len(index)}')
    ...
    def __getitem__(self, idx):
        ...
        return item_pdb_code, [pw_opposition, other_2345_homologies], binding_affinity

def process_homologies_into_tensor(homologies):
    # Process [pw_opposition, other_2345_homologies] into a tensor of shape (72, 201)
    ...
    # Finally, concatenate the pairwise homologies and the 2345 homologies to end up with a tensor of shape (72, 201)
...
class WeiDataset(ProteinPersistenceHomologyDataset):
    ...
    def __init__(self,
        ...
        use_only_existent_entries=True,
        transpose=False,
        return_pdb_code_first=False):

        self.normalize = normalize
        self.transpose = transpose
        self.return_pdb_code_first = return_pdb_code_first

        super().__init__(index, homology_base_folder, use_only_existent_entries)

    def __getitem__(self, idx):
        item_pdb_code, homologies, binding_affinity = super().__getitem__(idx)
        out = process_homologies_into_tensor(homologies)
        ...
        if self.return_pdb_code_first:
            return item_pdb_code, out, binding_affinity
        else:
